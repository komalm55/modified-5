...
function loadLeaveRequests() {
    ...
    const requestContainer = document.getElementById("leaveRequests");

    if (leaveRequest && !leaveRequest.approvedBy.includes("teacher")) {
...
function approveLeave(role) {
    ...
    if (!leaveRequest.approvedBy.includes(role)) {
        ...
        alert("✅ Leave approved by Teacher! Forwarding to HOD...");
        forwardToHOD(); // Forward to HOD
        loadLeaveRequests();
        closeModal();
    }
}
...
function forwardToHOD() {
    ...
    leaveRequest.status = "Pending Approval by HOD";
    localStorage.setItem("leaveRequest", JSON.stringify(leaveRequest));
